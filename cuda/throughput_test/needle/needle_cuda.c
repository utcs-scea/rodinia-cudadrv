        ...
        int max_cols, int penalty, int i, int block_width)
{
    ...
    sem_t *mutex;
    struct timestamp ts_throughput;
    float throughput_time;
    int loop;
    int outer;
    if ((mutex = sem_open("cuda_sem", O_CREAT, 0777, 1)) == SEM_FAILED) {
        perror("semaphore initilization");
        exit(1);
    }
    printf("[throughput_test] wait at barrier\n");
    sem_post(mutex);
    printf("[throughput_test] start\n");

    /* shared memory size is known in the kernel image. */
    const int loop_num = 3000;
    for (outer = 0; outer < 10; outer++) {
        probe_time_start(&ts_throughput);
        for (loop = 0; loop < loop_num; loop++)
            res = cuLaunchKernel(f, gdx, gdy, 1, bdx, bdy, 1, 0, 0, (void**) param, NULL);
        throughput_time = probe_time_end(&ts_throughput);
        printf("throughput_time: total=%f, each=%f\n", throughput_time, throughput_time / loop_num);
    }
