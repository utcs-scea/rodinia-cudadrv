...
__kernel void nw_kernel1(__global int* reference_d,
    __global int* input_itemsets_d, __global int* output_itemsets_d,
    __local int* input_itemsets_l, __local int* reference_l, int cols,
    int penalty, int blk, int block_width, int worksize, int offset_r,
    int offset_c)
{
    for (int k = 0; k < NUM_ITER; k++) {
        ...
        int index = base + cols * BLOCK_SIZE * b_index_y
            + BLOCK_SIZE * b_index_x + tx + (cols + 1);
        int index_n = base + cols * BLOCK_SIZE * b_index_y
            + BLOCK_SIZE * b_index_x + tx + (1);
        int index_w = base + cols * BLOCK_SIZE * b_index_y
            + BLOCK_SIZE * b_index_x + (cols);
        int index_nw = base + cols * BLOCK_SIZE * b_index_y
        ...
        for (int m = 0; m < BLOCK_SIZE; m++) {
            if (tx <= m) {
                ...
                SCORE(t_index_y, t_index_x) = maximum(
                    SCORE((t_index_y - 1), (t_index_x - 1))
        ...
        for (int ty = 0; ty < BLOCK_SIZE; ty++)
            input_itemsets_d[index + cols * ty]
                = SCORE((ty + 1), (tx + 1));
    ...
    int offset_c)
{
    for (int k = 0; k < NUM_ITER; k++) {
        ...
        for (int ty = 0; ty < BLOCK_SIZE; ty++)
            input_itemsets_d[index + ty * cols]
                = SCORE((ty + 1), (tx + 1));
